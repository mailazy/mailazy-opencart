...
class Mailazy {
    ...
    public function send() {
        ...
        $mailazyClient->setApikey($this->smtp_username);
        $mailazyClient->setApisecret($this->smtp_password);
        ...
        foreach ($this_to as $mail_to) {
            $mailazyClient->addAddress(trim($mail_to));
        }
        ...
        $mailazyClient->setSubject($this->subject);
        $mailazyClient->setBody($message);
        $mailazyClient->setFrom($this->from, $this->sender);
        $mailazyClient->addReplyTo($reply_to);

        if (is_array($this->attachments) && count($this->attachments) > 0) {
            foreach ($this->attachments as $attachment) {
                if (file_exists($attachment)) {
                    $mailazyClient->addAttachment($attachment);
                }
            }
        }
        ...
        $mailazyResponse = $mailazyClient->send();
        //echo json_encode($mailazyResponse);
    }
